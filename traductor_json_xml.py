	...
	def translating(self):
		...
		while self.absolute_position != len(self.separated_simbols)-1:
			...
			if self.lexer_token == 'r_llave':
				if closures_llave:
					if closures_llave[-1:][0][-1:][0] == self.tabs_numbers:
						self.increase_tabs(-1)
						self.write_file(closures_llave[-1:][0][0])
						closures_corchete = closures_llave[:-1]
				else:
					self.increase_tabs(-1)
					string = "</item>"
					self.write_file(string)
			...
			if self.lexer_token == 'r_corchete':
				if closures_corchete:
					if closures_corchete[-1:][0][-1:][0] == self.tabs_numbers:
						self.increase_tabs(-1)
						self.write_file(closures_corchete[-1:][0][0])
						closures_corchete = closures_corchete[:-1]
				else:	
					self.increase_tabs(-1)
					string = "</item>"
					self.write_file(string)
			...
			if self.lexer_token == 'string':
				...
				if self.lexer_token == 'dos_puntos':
					...
					if self.lexer_token == 'l_corchete':
						...
						string.insert(1, '/')
						token_cierre = string
						closures_corchete.append([''.join(token_cierre), self.tabs_numbers])
